...
export class NewChartsWidget8Component implements OnInit {
  ...
  constructor(private cdr: ChangeDetectorRef, public salesService: SalesService) {}
    ...
    this.salesService.reportSaleForCategories(data).subscribe((resp:any) => {
      ...
      this.report_sale_for_categories.sale_for_categories.forEach((element:any) =>{
        series_data.push({
          ...
          data: [[
            element.categories_avg,
            element.categories_total,
            element.categories_quantity,
          ]]
        });
      })
...
function getChart1Options(chartHeightNumber: number, series_data:Array<any>) {
  //x numero de ventas de categorias
  //y total de ventas de la categoria
  //z promedio de la categoria
  ...
  const options = {
    ...
    xaxis: {
      ...
      min: 0,
      title: {
        text: 'Cantidad de ventas de categorías',
        style: {
    ...
    yaxis: {
      ...
      // max: 7000,
      title: {
        text: 'Total de ventas por categoría (ARS)',
        style: {
          color: getCSSVariableValue('--bs-gray-700'),
          fontSize: '14px',
          fontWeight: 600,
